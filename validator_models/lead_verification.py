...
async def verify_stage5_unified(
    ...
    Uses a SINGLE LLM call to verify all three fields, similar to Stage 4.
    ...
    # Build context from DDG search results
    search_context = ""
    if ddg_search_results:
        for i, result in enumerate(ddg_search_results[:5], 1):
            title = result.get("title", "")
            snippet = result.get("snippet", result.get("body", ""))
            search_context += f"\n{i}. {title}\n   {snippet[:200]}"
    
    # Single LLM prompt for all three verifications
    prompt = f"""You are verifying B2B lead data quality. Analyze the provided information and verify THREE claims.
...
CLAIMS TO VERIFY:
1. ROLE: "{claimed_role}"
2. REGION: "{claimed_region}" (company HQ or operating region)
3. INDUSTRY: "{claimed_industry}" (sub-industry: "{claimed_sub_industry}")

SEARCH RESULTS (from DDG/LinkedIn):
{search_context if search_context else "(No search results provided)"}

VERIFICATION RULES:

1. ROLE VERIFICATION:
   - Extract the actual role from search results/LinkedIn title
   - Compare to claimed role "{claimed_role}"
   - Allow variations: "CEO" = "Chief Executive Officer", "Co-Founder & CEO" ≈ "CEO"
   - FAIL if fundamentally different (e.g., "Engineer" vs "CEO")

2. REGION VERIFICATION:
   - Use your knowledge of where {company} is headquartered
   - Compare to claimed region "{claimed_region}"
   - PASS if city matches OR state/country matches for that company
   - For remote-first companies, be lenient on region
   - Examples: "San Jose, CA" ≈ "San Jose, California, United States"

3. INDUSTRY VERIFICATION:
   - Determine what industry {company} operates in
   - Compare to claimed industry "{claimed_industry}"
   - Allow reasonable mappings: "Fintech" → "Finance and Insurance" ✓
   - FAIL if completely wrong (e.g., "Agriculture" for a software company)

RESPOND WITH JSON ONLY:
{{
    "role_match": true/false,
    "extracted_role": "actual role from profile/search",
    "region_match": true/false,
    "extracted_region": "actual company HQ location",
    "industry_match": true/false,
    "extracted_industry": "actual industry category",
    "confidence": 0.0-1.0,
    "reasoning": "Brief explanation of all three checks"
}}"""
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                # Overall verification: all three should match for full pass
                # But we return individual results for scoring flexibility
